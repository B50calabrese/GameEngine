 ...
 public:
  ...
  void OnUpdate(double delta_time) override {
    ...
    if (ActionManager::Get().IsStarted("Shake")) {
      TweenManager::Get().Tween<float>(0.05f, 0.0f, 0.5f)
    ...
    if (ActionManager::Get().IsStarted("Flash")) {
      TweenManager::Get().Tween<float>(1.0f, 0.0f, 0.2f)
          .OnUpdate([](const float& value, float progress) {
    ...
    Renderer::Get().DrawRect(100.0f, 100.0f, 200.0f, 200.0f, 1.0f, 0.0f, 0.0f); // Red Box
    Renderer::Get().DrawRect(400.0f, 300.0f, 150.0f, 150.0f, 0.0f, 1.0f, 0.0f); // Green Box
    Renderer::Get().DrawRect(200.0f, 400.0f, 100.0f, 100.0f, 0.0f, 0.0f, 1.0f); // Blue Box
  }
};
