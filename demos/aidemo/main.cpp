...
class AIDemoScene : public engine::Scene {
 public:
  ...
  void OnAttach() override {
    ...
    registry_.AddComponent<engine::core::TransformComponent>(npc_,
                                                     {{100, 100}, {1, 1}, 0});
    ...
    SetupFSM(registry_.GetComponent<engine::ecs::StateMachineComponent>(npc_).fsm,
             npc_);
    SetupBT(registry_.GetComponent<engine::ecs::BehaviorTreeComponent>(npc_).tree,
            npc_);

    registry_.GetComponent<engine::ecs::StateMachineComponent>(npc_).fsm.ChangeState(
        "Patrol");
  }
  ...
  void SetupFSM(engine::util::StateMachine& fsm, engine::ecs::EntityID entity) {
    ...
    // Flee State
    auto flee = std::make_shared<engine::util::LambdaState>([this, entity](float dt) {
      auto& transform =
          registry_.GetComponent<engine::core::TransformComponent>(entity);
      auto& npc = registry_.GetComponent<NPCComponent>(entity);
  ...
  void SetupBT(engine::util::BehaviorTree& tree, engine::ecs::EntityID entity) {
    ...
    auto is_patrolling = std::make_shared<engine::util::ConditionNode>(
        [this, entity](engine::util::Blackboard& bb) {
          auto& fsm =
              registry_.GetComponent<engine::ecs::StateMachineComponent>(entity).fsm;
  ...
  void OnUpdate(float dt) override { engine::ecs::AISystem::Update(&registry_, dt); }

  void OnRender() override {
    auto& transform = registry_.GetComponent<engine::core::TransformComponent>(npc_);
    auto& npc = registry_.GetComponent<NPCComponent>(npc_);

    // Draw NPC
    glm::vec4 color = {0, 1, 0, 1};  // Patrol
    auto& fsm = registry_.GetComponent<engine::ecs::StateMachineComponent>(npc_).fsm;
    if (fsm.GetCurrentStateName() == "Combat")
      color = {1, 0, 0, 1};
    else if (fsm.GetCurrentStateName() == "Flee")
      color = {1, 1, 0, 1};

    engine::graphics::Renderer::Get().DrawQuad(transform.position, {30, 30}, color);

    // Draw UI
    engine::graphics::Renderer::Get().DrawText("default", npc.status_text, {10, 570},
                                       0.0f, 0.7f, {1, 1, 1, 1});
