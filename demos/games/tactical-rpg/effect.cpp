...
std::string DamageEffect::GetLogMessage(Character* actor,
                                        Character* target) const {
  return actor->name + " hit " + target->name + " for " +
         std::to_string(last_damage_);
}
...
void HealEffect::Apply(Character* actor, Character* target) {
  ...
  target->stats.current_hp =
      std::min(target->stats.max_hp, target->stats.current_hp + heal);
  last_heal_ = heal;
}

std::string HealEffect::GetLogMessage(Character* actor,
                                      Character* target) const {
  return actor->name + " healed " + target->name + " for " +
         std::to_string(last_heal_);
}
