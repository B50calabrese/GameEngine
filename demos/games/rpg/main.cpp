...
class VictoryOverlay : public engine::Scene {
 public:
  ...
  void OnRender() override {
    ...
    engine::graphics::Renderer::Get().DrawText("default", "VICTORY!", {320, 350}, 0.0f,
                                       1.5f, {1, 1, 1, 1});
    engine::graphics::Renderer::Get().DrawText("default", "You cleared all 10 floors!",
                                       {250, 300}, 0.0f, 0.8f, {1, 1, 1, 1});
    engine::graphics::Renderer::Get().DrawText("default", "Press SPACE to Return",
                                       {300, 250}, 0.0f, 0.8f,
                                       {0.7f, 0.7f, 0.7f, 1.0f});
...
class BattleScene : public engine::Scene {
 public:
  ...
  void OnRender() override {
    ...
    // Enemy
    engine::graphics::Renderer::Get().DrawQuad({500, 300}, {100, 100}, {1, 0, 0, 1});
    ...
    // Player
    engine::graphics::Renderer::Get().DrawQuad({150, 150}, {80, 80}, {0, 1, 0, 1});
    engine::graphics::Renderer::Get().DrawText(
        "default",
    ...
    engine::graphics::Renderer::Get().DrawText("default", last_log_, {200, 50}, 0.0f,
                                       0.8f, {1, 1, 1, 1});
...
class LevelUpOverlay : public engine::Scene {
 public:
  ...
  void OnRender() override {
    ...
    if (!stat_picked_) {
      engine::graphics::Renderer::Get().DrawText("default",
                                         "LEVEL UP! Step 1: Choose Stat Boost",
                                         {200, 460}, 0.0f, 1.0f, {1, 1, 1, 1});
      std::vector<std::string> options = {"+1 ATK", "+1 DEF", "+5 Max HP"};
      for (int i = 0; i < 3; ++i) {
        glm::vec4 color = (selected_index_ == i) ? glm::vec4(1, 1, 0, 1)
                                                 : glm::vec4(1, 1, 1, 1);
        engine::graphics::Renderer::Get().DrawText("default", options[i],
                                           {250, 350 - (float)i * 40}, 0.0f,
                                           0.8f, color);
      }
    } else {
      engine::graphics::Renderer::Get().DrawText("default",
                                         "LEVEL UP! Step 2: Choose New Attack",
                                         {200, 460}, 0.0f, 1.0f, {1, 1, 1, 1});
      for (int i = 0; i < 3; ++i) {
        glm::vec4 color = (selected_index_ == i) ? glm::vec4(1, 1, 0, 1)
                                                 : glm::vec4(1, 1, 1, 1);
...
void MapScene::HandleTile(int x, int y, TileType type) {
  if (type == TileType::Chest) {
    ...
    if (GameState::Get().current_floor >= 10) {
      engine::SceneManager::Get().PushScene(std::make_unique<VictoryOverlay>());
    } else {
    ...
    if (std::uniform_real_distribution<>(0.0, 1.0)(gen) < chance) {
      engine::SceneManager::Get().PushScene(std::make_unique<BattleScene>("Battle"));
    }
  }
}
...
void MapScene::OnRender() {
  for (int y = 0; y < MAP_HEIGHT; ++y) {
    for (int x = 0; x < MAP_WIDTH; ++x) {
      ...
      engine::graphics::Renderer::Get().DrawQuad(pos, {TILE_SIZE - 2, TILE_SIZE - 2},
                                         color);
    }
  }
  ...
  void OnInit() override {
    std::vector<demos::common::BaseMenuScene::MenuItem> items = {
        {"New Run",
         []() {
           GameState::Get().ResetRun();
           engine::SceneManager::Get().SetScene(std::make_unique<MapScene>("Map"));
         }},
        {"Quit", []() { engine::Engine::Shutdown(); }}};
    engine::SceneManager::Get().SetScene(std::make_unique<demos::common::BaseMenuScene>(
        "RPG ROGUELIKE DEMO", items));
  }
