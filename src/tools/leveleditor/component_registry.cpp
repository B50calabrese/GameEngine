...
void ComponentRegistry::Init() {
  ...
  RegisterComponent({
      ...
      [](engine::ecs::EntityID entity, engine::ecs::Registry& reg, nlohmann::json& j) {
        auto& t = reg.GetComponent<Transform>(entity);
        j["position"] = {t.position.x, t.position.y};
        j["scale"] = {t.scale.x, t.scale.y};
        j["rotation"] = t.rotation;
      },
      [](engine::ecs::EntityID entity, engine::ecs::Registry& reg, const nlohmann::json& j) {
        Transform t;
  ...
  RegisterComponent({
      ...
      [](engine::ecs::EntityID entity, engine::ecs::Registry& reg) {
        ...
        if (ImGui::InputText("Texture", buf, sizeof(buf))) s.texture_name = buf;

        strncpy(buf, s.sprite_sheet_name.c_str(), sizeof(buf));
        if (ImGui::InputText("Sheet", buf, sizeof(buf))) s.sprite_sheet_name = buf;
