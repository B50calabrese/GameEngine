...
void PhysicsSystem::Update(Registry* registry, float dt) {
  ...
  for (size_t i = 0; i < colliders.size(); ++i) {
    for (size_t j = i + 1; j < colliders.size(); ++j) {
      ...
      auto& trans_a = registry->GetComponent<components::Transform>(a);
      auto& col_a = registry->GetComponent<components::Collider>(a);
      auto& trans_b = registry->GetComponent<components::Transform>(b);
      auto& col_b = registry->GetComponent<components::Collider>(b);
      ...
      if (util::CheckAABB(trans_a.position + col_a.offset, col_a.size,
        ...
        if (!col_a.is_trigger && !col_b.is_trigger) {
          if (!col_a.is_static &&
              registry->HasComponent<components::Velocity>(a)) {
            auto& vel_a = registry->GetComponent<components::Velocity>(a);
