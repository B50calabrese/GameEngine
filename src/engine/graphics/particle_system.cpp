...
ParticleSystem::ParticleSystem(size_t max_particles) : max_particles_(max_particles) {
    particles_.reserve(max_particles);
}

void ParticleSystem::Emit(const glm::vec2& position, int count,
                           const glm::vec2& base_velocity, const glm::vec2& velocity_range,
                           const glm::vec4& color, float lifetime_base) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<float> dis_vx(-velocity_range.x, velocity_range.x);
    std::uniform_real_distribution<float> dis_vy(-velocity_range.y, velocity_range.y);
