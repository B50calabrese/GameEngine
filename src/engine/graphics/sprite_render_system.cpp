...
void SpriteRenderSystem::Render(ecs::Registry& registry) {
  ...
  auto trans_view = registry.GetView<TransformComponent>();
  for (auto entity : trans_view) {
    ...
    if (registry.HasComponent<SpriteComponent>(entity)) {
      ...
      if (sprite.visible) {
        if (!sprite.sprite_sheet_name.empty()) {
          auto sheet = util::AssetManager<SpriteSheet>::Get(sprite.sprite_sheet_name);
          if (sheet) {
    ...
    // Quad Component (fallback)
      ...
      Renderer::Get().DrawQuad(transform.position, transform.scale, quad.color, transform.rotation);
    }
